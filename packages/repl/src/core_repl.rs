//! Platform-independent REPL core using new architecture.
//!
//! This module contains the main REPL logic using the new core-store architecture.

use crate::core_commands::{self, CommandResult};
use crate::core_store_context::StoreContext;
use crate::io::{ExitReason, IoError, IoHost, Output, PromptConfig, Signal};

/// The platform-independent REPL core (new architecture).
pub struct ReplCore {
    ctx: StoreContext,
}
...
impl ReplCore {
    ...
    pub fn run(&mut self, io: &mut impl IoHost) -> Result<ExitReason, IoError> {
        ...
        loop {
            ...
            let result = core_commands::execute(&input.line, &mut self.ctx);
            ...
            match result {
                ...
                CommandResult::Help => {
                    io.write_output(Output::normal(core_commands::format_help()))?;
                }
                CommandResult::Exit => {
                    io.write_output(Output::info("Goodbye!"))?;
                    io.flush()?;
...
[New Architecture] Type 'help' for available commands, 'exit' to quit.
"#;
