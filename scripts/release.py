...
def release_status(packages, lookup):
    """Registry evidence, not a tag or a claim inferred from an earlier release."""
    versions = {}
    for package in packages:
        entry = lookup(package["name"]).get(package["version"])
        versions[package["name"]] = {
            "version": package["version"],
            "status": "unpublished" if entry is None else ("yanked" if entry.get("yanked") else "published"),
        }
    return {"verified_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "source": "https://index.crates.io", "packages": versions}


def write_release_status(packages):
    status = release_status(packages, registry_versions)
    (ROOT / "docs/release-status.json").write_text(json.dumps(status, indent=2) + "\n")
    lines = ["# Registry release status", "", "Generated by `scripts/release.py --status`. This records publication only;",
             "validation and specification support are documented separately.", "",
             "Verified at " + status["verified_at_utc"] + ".", "",
             "| Package | Version | Registry status |", "| --- | --- | --- |"]
    for name, entry in status["packages"].items():
        lines.append(f"| {name} | {entry['version']} | {entry['status']} |")
    (ROOT / "docs/release-status.md").write_text("\n".join(lines) + "\n")


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    modes = parser.add_mutually_exclusive_group()
    modes.add_argument("--status", action="store_true", help="verify registry status and regenerate the documentation record; no publishing")
