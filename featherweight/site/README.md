# The featherweight site

The website for the featherweight project — the single-node reference
implementation of the [Isotope spec](../../isotope/spec). Follows the
same Eleventy + Cloudflare Pages pattern as `site/` (structfs.com),
`namecode/site/`, and `isotope/site/`.

## Build

```bash
./build.sh     # builds kv.wasm, copies the browser host in, runs 11ty
./dev.sh       # build, then serve with live reload
./clean.sh
```

Needs: Rust with the `wasm32-unknown-unknown` target, Node 20+, pnpm.

`src/demo/` is **copied in by build.sh** (the browser-host modules from
`featherweight/host/browser` plus the compiled kv guest) and gitignored
— the demo can never drift from the host. Edit the originals, never the
copies.

## The live demo and COOP/COEP

`/demo/` runs kv.wasm resident in a Web Worker, which needs
`SharedArrayBuffer` and therefore cross-origin isolation. `src/_headers`
applies COOP/COEP site-wide on Cloudflare Pages. The Eleventy dev server
sends no such headers, so locally the demo degrades to batch mode (each
request re-instantiates the block); `pnpm dlx wrangler pages dev _site`
serves the production headers if you need the resident mode locally.

Because of the site-wide `Cross-Origin-Embedder-Policy: require-corp`,
any future cross-origin embed (fonts, images, iframes from other
domains) must carry CORP/CORS headers or the browser will block it.

## Deploying

**The domain is TBD** — all site domains are single-sourced from
`/sites.json` at the repo root; edit the `featherweight` entry there
when the real domain is chosen and every cross-link (on this site and
its siblings) plus this site's canonical URL update together.

`.github/workflows/deploy-featherweight-site.yml` is
`workflow_dispatch`-only until the Cloudflare side exists. To go live:

1. Create a Cloudflare Pages project named `featherweight-site` (or
   edit the `--project-name` in the workflow).
2. Ensure the `structfs_com` environment secrets
   (`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`) cover it, or give
   the workflow its own environment.
3. Point the chosen domain at the Pages project and update the
   `featherweight` entry in `/sites.json`.
4. Add the push trigger to the workflow (see the comment in it).
